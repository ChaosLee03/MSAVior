/* L2_003.c  payload camera RS422  8051 */
/* VBUG00012863 */
#include <reg51.h>
#include <intrins.h>
#include <absacc.h>
...
static uchar CheckRS422Frame(void);
...
static uchar CheckRS422Frame(void)
{
    ...
    /* check condition */
    if ((CAMERA_A_REC_BUF[0] != RS422_SYNC_BYTE0)
     || (CAMERA_A_REC_BUF[1] != RS422_SYNC_BYTE1))
    {
    ...
    sum_check = CAMERA_A_REC_BUF[RS422_FRAME_LEN - 1u];
    if (sum != sum_check)
    {
    /* 控制量计算输出 */
...
void payload_RS422(void)
{
    ...
    {
        cmd_buf[i] = CAMERA_A_REC_BUF[CAM_CMD_FIELD_OFFSET + (uchar)i];
    }
    /* --- */


    frame_chk = CheckRS422Frame();
