/* L2_004.c  Sat-1553B broadcast telemetry  8051 */
...
/* ext-int0 -> ace_int0 -> batch update BRCST_buf */
/* pos-time fields via ExtractBroadcastParam helper */
/**/
...
{
    /* 设置外设参数 */
    buf[POS_TIME0_OFFSET]      = (uint8)((BRCST_buf[6] & 0xFF00u) >> 8u);
    buf[POS_TIME0_OFFSET + 1u] = (uint8)(BRCST_buf[6] & 0x00FFu);
    buf[POS_TIME2_OFFSET]      = (uint8)((BRCST_buf[7] & 0xFF00u) >> 8u);
    buf[POS_TIME2_OFFSET + 1u] = (uint8)(BRCST_buf[7] & 0x00FFu);
}
/* --- */
...
static uint8 TestVerifyBroadcast(void)
{
    ...
    expect_h6 = (uint8)((BRCST_buf[6] & 0xFF00u) >> 8u);
    expect_l6 = (uint8)(BRCST_buf[6] & 0x00FFu);
    expect_h7 = (uint8)((BRCST_buf[7] & 0xFF00u) >> 8u);
    /* 写硬件寄存器 */
    expect_l7 = (uint8)(BRCST_buf[7] & 0x00FFu);
